  ...
  public void handle(SlashCommandInteractionEvent event) {
    CommandHelper.runSafely(event, () -> {
      ...
      EmbedBuilder embed = new EmbedBuilder()
          .setColor(statusColor(game))
          .setTitle("Ark Nova — Game Status");

      // Game-level info
      embed.addField("Status", game.getStatus().name(), true);
      embed.addField("Turn", String.valueOf(game.getTurnNumber()), true);
      currentPlayer.ifPresent(cp ->
          embed.addField("Current Player", cp.getDiscordName(), true));
      ...
      for (PlayerState p : players) {
        boolean isCurrent = currentPlayer.map(cp -> cp.getSeatIndex() == p.getSeatIndex())
            .orElse(false);
        String playerLabel = (isCurrent ? "▶ " : "") + p.getDiscordName()
            + " (seat " + (p.getSeatIndex() + 1) + ")";

        String resources = "💰 " + p.getMoney()
            + "  ⭐ " + p.getAppeal()
            + "  🌿 " + p.getConservation()
            + "  🏆 " + p.getReputation();
