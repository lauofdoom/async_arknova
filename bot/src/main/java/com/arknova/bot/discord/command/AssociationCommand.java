 ...
 *
 * <ul>
 *   <li>{@code sub_actions} — comma-separated task types (required): PARTNER_ZOO | UNIVERSITY |
 *       CONSERVATION_PROJECT | RETURN_WORKERS
 *   <li>{@code project_ids} — comma-separated conservation project card IDs (one per
 *       CONSERVATION_PROJECT entry)
  ...
  public SubcommandData getSubcommandData() {
    return new SubcommandData("association", "Perform association board tasks")
        .addOption(
            OptionType.STRING,
            "sub_actions",
            "Tasks: PARTNER_ZOO, UNIVERSITY, CONSERVATION_PROJECT, RETURN_WORKERS",
            true)
        .addOption(
            OptionType.STRING,
            "project_ids",
            "Conservation project card IDs (one per CONSERVATION_PROJECT, comma-separated)",
            false)
        .addOption(
            OptionType.INTEGER,
            "donation_amount",
            "Money to donate (upgraded only; multiple of 3)",
            false);
  }
    ...
    CommandHelper.runSafely(
        ...
        () -> {
          ...
          List<String> subActions =
              CommandHelper.splitCsv(event.getOption("sub_actions", OptionMapping::getAsString));
