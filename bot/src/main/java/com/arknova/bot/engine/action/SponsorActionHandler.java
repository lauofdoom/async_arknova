 ...
 * <ul>
 *   <li>{@code "card_ids"}         — ordered list of sponsor card IDs from hand to play</li>
 *   <li>{@code "display_card_ids"} — sponsor card IDs from the display (upgraded only)</li>
 * </ul>
 *
 * <h2>Request parameters — break</h2>
 * <ul>
 *   <li>{@code "break"} — set to {@code true} to break instead of playing cards</li>
 * </ul>
 */
@Component
...
public class SponsorActionHandler implements ActionHandler {
  ...
  public ActionResult execute(ActionRequest request, PlayerState player,
    ...
    if (!isBreak) {
      ...
      isBreak = Boolean.TRUE.equals(breakParam) || "true".equalsIgnoreCase(String.valueOf(breakParam));
    }
    ...
    for (String cardId : handCardIds) {
      ...
      ActionResult err = validateSponsor(pc.getCard(), cardId, maxLevel, strength,
          player.getMoney() - pendingCost, plays, false, 0);
    ...
    for (String cardId : displayCardIds) {
      ...
      ActionResult err = validateSponsor(pc.getCard(), cardId, maxLevel, strength,
          player.getMoney() - pendingCost, plays, true, cost);
      if (err != null) return err;
      pendingCost += cost;
    ...
    for (int i = 0; i < plays.size(); i++) {
      ...
      summary.append("**").append(p.cardDef().getName()).append("**")
          .append(" (lvl ").append(p.cardDef().getBaseCost()).append(")")
          .append(" for ").append(p.cost()).append("💰");
      if (p.fromDisplay()) summary.append(" (display)");
    }
    summary.append(".");
    if (totalAppeal       > 0) summary.append(" +").append(totalAppeal).append(" appeal.");
    if (totalConservation > 0) summary.append(" +").append(totalConservation).append(" conservation.");
    ...
    log.info("Game {}: {} SPONSOR strength={} played={} cost={}", request.gameId(),
        discordId, strength, plays.size(), totalCost);

    return new ActionResult(
        true, null, ActionCard.SPONSOR, strength, summary.toString(),
        Map.of("money_spent",         totalCost,
               "appeal_gained",       totalAppeal,
               "conservation_gained", totalConservation,
               "reputation_gained",   totalReputation,
               "cards_played",        plays.size()),
        List.of(), false, anyManual, firstManualId);
  }

  // ── Break ─────────────────────────────────────────────────────────────────────

  private ActionResult executeBreak(ActionRequest request, PlayerState player,
      int strength, boolean upgraded) {

    int gained = upgraded ? strength * 2 : strength;
    player.setMoney(player.getMoney() + gained);

    String summary = request.discordName() + " broke **Sponsor** (X=" + strength + ") → +"
        + gained + "💰" + (upgraded ? " (2×X, upgraded)" : "") + ".";

    log.info("Game {}: {} SPONSOR break strength={} gained={}", request.gameId(),
        request.discordId(), strength, gained);

    return ActionResult.success(ActionCard.SPONSOR, strength, summary,
        Map.of("money_gained", gained, "break", true));
  }

  // ── Validation ────────────────────────────────────────────────────────────────

  /** Returns a failure result if invalid, null if OK. */
  private ActionResult validateSponsor(CardDefinition def, String cardId,
      int maxLevel, int strength, int remainingMoney,
      List<SponsorPlay> pending, boolean fromDisplay, int displayCost) {
    ...
    if (def.getBaseCost() > maxLevel) {
      return ActionResult.failure(
          def.getName() + " has level " + def.getBaseCost()
          + " but the maximum level you can play at strength " + strength
          + " is " + maxLevel + ".");
    }
    int cost = fromDisplay ? displayCost : def.getBaseCost();
    if (remainingMoney < cost) {
      return ActionResult.failure(
          def.getName() + " costs " + cost + "💰 but you only have "
