...
/**
 * Contract for the five action card handlers. Each implementation handles one of:
 * CARDS, BUILD, ANIMALS, ASSOCIATION, SPONSOR.
 *
 * <p>Handlers receive already-validated inputs (turn validation and snapshot are handled by
 * {@link com.arknova.bot.engine.GameEngineImpl} before dispatching here). Handlers are
 * responsible for:
 * <ol>
...
public interface ActionHandler {
  ...
  /**
   * Execute the action. The strength used is available via
   * {@link PlayerState#getStrengthOf(ActionCard)} before the card rotation is applied.
   *
