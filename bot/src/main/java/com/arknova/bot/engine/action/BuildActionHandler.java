 ...
 *
 * Base cost = size × 2 money. E.g. size 3 = 6 money, size 5 = 10 money. Optional terrain tags
 * (WATER, ROCK) add +2 money each.
 *
...
public class BuildActionHandler implements ActionHandler {
  ...
  public ActionResult execute(
    ...
    List<String> terrainTags =
        request.paramList("tags").stream()
            .map(String::toUpperCase)
            .filter(TERRAIN_TAGS::contains)
            .toList();

    if (size < 1) {
      return ActionResult.failure("Please specify an enclosure size (1–" + strength + ").");
    }
    if (row < 0 || col < 0) {
    ...
    return buildSingle(
        request, player, strength, size, row, col, terrainTags, existingEnclosureCount(player));
  }

  // ── Multi-build ───────────────────────────────────────────────────────────────

  private ActionResult executeMultiBuild(
    ...
    int totalSize =
        buildings.stream().mapToInt(b -> ((Number) b.getOrDefault("size", 0)).intValue()).sum();
    ...
    for (int i = 0; i < buildings.size(); i++) {
      ...
      if (player.getMoney() < pendingCost) {
        return ActionResult.failure(
            "Insufficient money: total build cost so far is "
                + pendingCost
                + " but you only have "
                + player.getMoney()
                + ".");
    ...
    log.info(
        "Game {}: {} multi-built {} enclosure(s) total-size={} cost={}",
        request.gameId(),
        request.discordId(),
        plans.size(),
        totalSize,
  ...
  private ActionResult buildSingle(
      ActionRequest request,
      PlayerState player,
      int strength,
      int size,
  ...
  private ActionResult executeUpgrade(
      ActionRequest request, PlayerState player, int strength, String targetCardName) {

    if (strength < 4) {
      return ActionResult.failure(
          "Upgrading an action card requires BUILD strength 4 or higher (current: "
              + strength
              + ").");
    }
  ...
  @SuppressWarnings("unchecked")
  private void addEnclosureToBoard(
      PlayerState player, String enclosureId, int size, int row, int col, List<String> tags) {
    try {
      Map<String, Object> board =
          objectMapper.readValue(player.getBoardState(), new TypeReference<>() {});
      List<Map<String, Object>> enclosures =
          new ArrayList<>((List<Map<String, Object>>) board.getOrDefault("enclosures", List.of()));
