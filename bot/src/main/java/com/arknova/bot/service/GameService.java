...
public class GameService {
   ...
   * @param gameId the game
   * @param targetDiscordId the player to adjust
   * @param track track name (case-insensitive)
   * @param setValue if non-null, set the track to this absolute value
   * @param deltaValue if non-null, add this delta (may be negative)
   * @return the updated PlayerState
   * @throws IllegalStateException if the player is not found or the track name is unrecognised
   */
  @Transactional
  public PlayerState adjustTrack(
      UUID gameId, String targetDiscordId, String track, Integer setValue, Integer deltaValue) {
    ...
    switch (track.toLowerCase()) {
      case "appeal" -> {
        int cur = player.getAppeal();
        player.setAppeal(setValue != null ? setValue : Math.max(0, cur + deltaValue));
      }
      case "conservation" -> {
        int cur = player.getConservation();
        player.setConservation(setValue != null ? setValue : Math.max(0, cur + deltaValue));
      }
      case "reputation" -> {
        int cur = player.getReputation();
    ...
    player = playerStateRepo.save(player);
    log.info(
        "Track '{}' adjusted for player {} in game {} (set={}, delta={})",
        track,
        targetDiscordId,
        gameId,
        setValue,
