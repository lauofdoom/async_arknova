 ...
 * <h2>Hex geometry</h2>
 *
 * <b>Flat-top</b> orientation — each hex has a flat horizontal edge at top and bottom, with pointed
 * vertices on the left and right sides. This matches the physical Ark Nova player mat. Hexes tile
 * in <b>offset-column</b> layout: adjacent columns step right by 3R/2 and odd-numbered columns are
 * shifted downward by half a hex-height (√3R/2).
 *
 * <h2>Grid coordinates</h2>
 *
 * {@code col} 0–8 runs left→right; {@code row} 0–6 runs top→bottom within each column. The board is
 * not a full rectangle — a {@link #COL_ROW_RANGE} mask clips the four corners to give the
 * rounded-hexagonal outline of the physical player mat.
 *
 * <h2>Enclosures</h2>
 *
 * Each enclosure occupies {@code size} consecutive hexes at columns {@code col}, {@code col+1}, …,
 * {@code col+size-1} in the same {@code row}. Because odd columns are offset down, multi-hex
 * enclosures have a slight visual zigzag, which mirrors the physical tile placement on the board.
 *
 ...
 *   <li>WATER — steel blue
 *   <li>ROCK — stone grey
 *   <li>WATER+ROCK— blue-grey
 *   <li>Any with animal placed — darker variant
 * </ul>
 */
@Component
...
public class ZooBoardRenderer {
  ...
  /**
   * Board-shape mask: valid row range {@code [lo, hi]} (inclusive) per column. All 9 columns span
   * all 7 rows — the board is a full 7×9 rectangle of hexes. The natural offset-column zigzag at
   * the top and bottom edges is inherent to the flat-top hex grid and does not represent missing
   * cells.
   */
  ...
  private BufferedImage renderBoard(PlayerState player) {
    ...
    // Grid section header
    sectionHeader(
        g, "Zoo Board  (col 0–" + (COLS - 1) + ", row 0–" + (ROWS - 1) + ")", GRID_X, GRID_Y - 10);
  ...
  /**
   * Builds a <b>flat-top</b> hexagon {@link Polygon} centred at (cx, cy). Vertices are at angles
   * 0°, 60°, 120°, 180°, 240°, 300° from the positive x-axis, giving a flat horizontal edge at the
   * top (between the 60° and 120° vertices) and a flat horizontal edge at the bottom (between the
   * 240° and 300° vertices).
   */
  private static Polygon hexPolygon(double cx, double cy) {
    ...
    for (int i = 0; i < 6; i++) {
      ...
      poly.addPoint(
          (int) Math.round(cx + HEX_R * Math.cos(a)), (int) Math.round(cy + HEX_R * Math.sin(a)));
    }
    return poly;
  }
  ...
  private void drawEnclosure(Graphics2D g, PlacedEnclosure enc) {
    ...
    g.setColor(Color.WHITE);
    g.setFont(font(Font.BOLD, 11));
    g.drawString(enc.id(), (int) (first[0] - HEX_R * 0.8), (int) (first[1] - HEX_H * 0.28));

    // Size — lower-left of first hex
    g.setColor(new Color(255, 255, 200, 220));
    g.setFont(font(Font.PLAIN, 9));
    g.drawString(
        "sz" + enc.size(), (int) (first[0] - HEX_R * 0.8), (int) (first[1] + HEX_H * 0.25));
  ...
  private void drawInfoPanel(Graphics2D g, PlayerState player) {
    ...
    resource(
        g, "Workers", player.getAssocWorkersAvailable() + "/" + player.getAssocWorkers(), 415, ry);
  ...
  private void compactTrackBar(
      Graphics2D g, String label, int value, int max, int barW, int x, int y) {
  ...
  private record PlacedEnclosure(
      String id, int size, int row, int col, String[] tags, List<String> animalCardIds) {}
}
